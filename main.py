...
async def websocket_endpoint(websocket: WebSocket):
    ...
    target_model = "models/gemini-2.0-flash-exp"  # Único motor WebSocket Live nativo de audio/video de Google
    ...
    try:
        async with websockets.connect(url) as gemini_ws:
            ...
            # Adaptación dinámica del prompt del sistema según el modelo elegido para simular Flash/Pro
            profundidad_instruccion = (
                "Señor ha solicitado un INFORME PROFUNDO. Proporciona análisis de Order Flow muy detallados de la estructura, mechas y desequilibrios."
                if modelo == "pro" else
                "Señor busca RESPUESTAS ULTRA RÁPIDAS. Sé sumamente conciso, limita tus respuestas a un máximo de 1 o 2 oraciones."
            )
